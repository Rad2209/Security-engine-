...
function ProductListPage() {
        ...
        if (!cancelled) {
          setError(
            err.response?.data?.error?.message ||
              'Could not load products. Is the backend running and reachable?'
          );
          setResult({ items: [], pagination: { page: 1, totalPages: 1 } });
        }
      })
      .finally(() => {
        if (!cancelled) setIsLoading(false);
  ...
  return (
    <div className="mx-auto max-w-6xl px-6 py-12">
      ...
      {error && (
        <p className="mt-6 rounded-sm border border-tick-red/40 bg-tick-red/10 px-4 py-3 font-mono text-xs text-tick-red">
          {error}
        </p>
      )}
